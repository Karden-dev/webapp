...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderReportsTable = (reports) => {
        ...
        reportsToRender.forEach((report, index) => {
            ...
            // --- NOUVELLE LOGIQUE POUR LE STYLE DU BOUTON ---
    ...
    reportsTableBody?.addEventListener('click', async (e) => {
        ...
        try {
            ...
            // --- NOUVELLE LOGIQUE POUR METTRE À JOUR LE BOUTON APRÈS LE CLIC ---
            const storageKey = `copied-report-${reportDate}-${shopId}`;
            sessionStorage.setItem(storageKey, 'true'); // Marquer comme copié

            // Mettre à jour visuellement le bouton sans recharger
