...
// const shopProspectModel = require('./models/shop.prospect.model'); // SUPPRIMÉ
...
// -- Services Communication & Archivage --
const WhatsAppService = require('./services/whatsapp.service');
// const AIService = require('./services/ai.service'); // SUPPRIMÉ
// const AIWatcher = require('./scripts/aiAgentWatcher'); // SUPPRIMÉ
const archiveService = require('./scripts/archiveConversations'); // CONSERVÉ
...
app.use('/api/whatsapp/webhook', express.raw({
    ...
    verify: (req, res, buf) => {
        req.rawBody = buf;
        console.log("[Webhook Middleware] Raw body capturé.");
    }
}));
...
// --- Fichiers Statiques ---
// 1. Dossier public général
app.use(express.static(path.join(__dirname, '..', 'public')));
// 2. Dossier uploads spécifique (Images produits/preuves)
...
async function startServer() {
    ...
    // 2. Initialisation des Modèles (Injection de dépendance)
    console.log('🔄 Initialisation des Modèles...');
    userModel.init(dbPool);
    shopModel.init(dbPool);
    // shopProspectModel.init(dbPool); // SUPPRIMÉ
    ...
    reportService.init(dbPool);
    firebaseService.initialize();
    
    // -- Services Communication & Archivage --
    WhatsAppService.init(dbPool);
    // AIService.init(dbPool); // SUPPRIMÉ
    // AIWatcher.init(dbPool); // SUPPRIMÉ
    archiveService.init(dbPool); // CONSERVÉ
