...
// --- NOUVELLE FONCTION : RECONSTITUTION CIBLÉE MARCHAND ---
const reconstituteMerchant = async (req, res) => {
    try {
        const { date, shopId } = req.body;

        if (!date || !shopId) {
            return res.status(400).json({ message: "La date et l'ID du marchand sont requis." });
        }

        // Appel au service dédié pour la reconstruction robuste
        const result = await reportService.reconstituteMerchantReport(date, shopId);

        res.status(200).json(result);

    } catch (error) {
        console.error("Erreur (reconstituteMerchant):", error);
        res.status(500).json({ message: 'Erreur lors de la reconstitution du rapport marchand.', error: error.message });
    }
};
...
module.exports = {
    ...
    processStorage,
    recalculateReport,
    reconstituteMerchant, // <-- AJOUT EXPORT
    exportPdf
};