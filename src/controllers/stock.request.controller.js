...
const declareEntry = async (req, res) => {
    try {
        ...
        const requestId = await stockRequestModel.create({
            ...
            proof_image_url: proof_url || null, 
        ...
        // Afficher plus de détails dans la réponse pour le débogage si besoin
        res.status(500).json({ message: "Erreur lors de la déclaration.", error: error.message });
    }
};
