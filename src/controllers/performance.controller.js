...
const calculatePiedRemuneration = (stats) => {
    ...
    // NOUVEAU : Variables pour la gestion du Malus
    ...
    // NOUVEAU : Appliquer le Malus si les dépenses dépassent 40% du CA
    if (ca > 0 && expenseRatio > 0.40) {
        const allowedExpenses = ca * 0.40; // Plafond de dépenses autorisé (40%)
        const excess = expenses - allowedExpenses; // Montant excédentaire
        
        if (excess > 0) {
            penaltyApplied = true;
            penaltyAmount = excess;
            finalPay -= penaltyAmount; // Déduction de l'excédent sur le salaire
        }
    }
    ...
    return {
        ...
        penaltyApplied: penaltyApplied, // NOUVEAU : Indique si le malus a été appliqué
        penaltyAmount: penaltyAmount,   // NOUVEAU : Montant déduit
...
const calculateMotoRemuneration = (livreurDetails, stats, objectivesAdmin = null) => {
    ...
    const target = objectivesAdmin?.target_deliveries_moto || 350; // Objectif par défaut si non défini
    ...
    const bonusTiers = objectivesAdmin?.bonus_tiers_moto || [
        { min_percent: 50, bonus: 50 }, { min_percent: 60, bonus: 75 },
        { min_percent: 70, bonus: 100 }, { min_percent: 80, bonus: 125 }
    ];
    // Le seuil d'éligibilité dépend du premier palier défini
    const eligibilityThresholdPercent = bonusTiers[0]?.min_percent || 50;
