...
// CORRECTION ICI: Importe 'isRider' au lieu de 'isLivreur'
...
// --- NOUVELLE ROUTE FCM POUR LA RIDERAPP ---

/**
 * @route   PATCH /api/deliverymen/fcm-token
 * @desc    Enregistre le token FCM de l'appareil pour l'utilisateur authentifié (livreur).
 * @access  Privé (Livreur uniquement)
 */
if (deliverymenController && deliverymenController.updateFcmToken) {
    // CORRECTION ICI: Utilise 'isRider' au lieu de 'isLivreur'
    router.patch('/fcm-token', verifyToken, isRider, deliverymenController.updateFcmToken);
} else {
    console.error("ERREUR DE CONFIGURATION: deliverymenController.updateFcmToken manquant.");
     // CORRECTION ICI: Utilise 'isRider' au lieu de 'isLivreur'
    router.patch('/fcm-token', verifyToken, isRider, (req, res) => res.status(501).json({ message: "Fonctionnalité (update FCM Token) non implémentée." }));
}
