...
const findAll = async (filters) => {
    ...
    try {
        ...
        // --- DÉBUT DE LA LOGIQUE MODIFIÉE ---

        // Cas 1: La requête concerne les dettes "En attente"
        // On filtre sur la date de CRÉATION (created_at)
        ...
        // Cas 2: La requête concerne les dettes "Réglées"
        // On filtre sur la date de RÈGLEMENT (settled_at) en utilisant les nouveaux paramètres
        ...
        // Cas 3: Requête générique sans statut (ne devrait pas arriver avec la nouvelle logique, mais on garde par sécurité)
        else {
             if (filters.startDate) {
                query += ` AND d.created_at >= ?`;
                params.push(moment(filters.startDate).startOf('day').format('YYYY-MM-DD HH:mm:ss'));
