 ...
 * Retourne un OBJET avec le total et les détails.
 * @param {string} startDate
 * @param {string} endDate
 * @returns {Promise<{total: number, delivery: number, packaging: number, storage: number}>}
 */
const getRevenue = async (startDate, endDate) => {
    ...
    try {
        ...
        const delivery = parseFloat(revenueResult.total_delivery_fees || 0);
        const packaging = parseFloat(revenueResult.total_packaging_fees || 0);
        const storage = parseFloat(revenueResult.total_storage_fees || 0);
        
        const total = delivery + packaging + storage;

        // On retourne un objet détaillé au lieu d'un simple chiffre
        return { total, delivery, packaging, storage };
