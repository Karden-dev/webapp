 ...
 * MODIFICATION: Ajout filtre "non lu" admin et Tri "WhatsApp" ROBUSTE
 */
const findConversationsForAdmin = async (adminUserId, filters) => {
    ...
    try {
            ...
            SELECT
                ...
                (SELECT COUNT(*)
                   ...
                   WHERE om_unread.order_id = o.id
                     ...
                     -- // MODIFICATION: Filtre des messages non lus pour l'admin
                     AND (
                        om_unread.message_type = 'user'
                        OR (
                            om_unread.message_type = 'system'
                            AND (
                                om_unread.message_content LIKE '%Livré%' OR
                                om_unread.message_content LIKE '%raté%' OR 
                                om_unread.message_content LIKE '%relancer%' OR
                                om_unread.message_content LIKE '%Reportée%' OR
                                om_unread.message_content LIKE '%Injoignable%' OR
                                om_unread.message_content LIKE '%Ne décroche pas%' OR
                                om_unread.message_content LIKE '%Annulé%'
                            )
                        )
                     )
        ...
        // // MODIFICATION ROBUSTE: Tri style "WhatsApp"
        // Force le 'unread_count' à être traité comme un nombre signé.
        query += ' ORDER BY CAST(unread_count AS SIGNED) DESC, last_message_time DESC';
        // // FIN MODIFICATION
 ...
 * MODIFICATION: Ajout d'une logique de filtre pour le rôle 'admin'.
 */
const countAllUnreadMessages = async (userId, userRole) => {
     ...
     try {
         ...
         // // MODIFICATION: Logique de filtre pour admin
         } else if (userRole === 'admin') {
            condition = `
                AND (
                    om.message_type = 'user'
                    OR (
                        om.message_type = 'system'
                        AND (
                            om.message_content LIKE '%Livré%' OR
                            om.message_content LIKE '%raté%' OR
                            om.message_content LIKE '%relancer%' OR
                            om.message_content LIKE '%Reportée%' OR
                            om.message_content LIKE '%Injoignable%' OR
                            om.message_content LIKE '%Ne décroche pas%' OR
                            om.message_content LIKE '%Annulé%'
                        )
                    )
                )
            `;
