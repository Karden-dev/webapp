...
module.exports = {
    ...
    findAll: async (filters) => {
        ...
        try {
            ...
            // --- AJOUT : Filtre par Boutique (Sécurité Marchand) ---
            if (filters.shopId) {
                query += ` AND o.shop_id = ?`;
    ...
    /**
     * Récupère les commandes livrées, prêtes pour la 1ère étape de demande d'avis.
     * (MODIFIÉ pour inclure le nom de la boutique et vérifier le drapeau de conversation)
     */
        ...
        try {
            const query = `
                SELECT 
                    ...
                    o.customer_phone,
                    o.delivery_location,
                    o.article_amount, -- AJOUTÉ POUR LE PRIX
                ...
                LEFT JOIN users u ON o.deliveryman_id = u.id
                LEFT JOIN shops s ON o.shop_id = s.id
                LEFT JOIN customer_conversation_state ccs ON o.customer_phone = ccs.customer_phone
                WHERE 
                    o.status = 'delivered'
                    AND o.ai_review_sent = 0
                   -- NOUVELLE LOGIQUE TEMPORELLE: Commandes livrées entre 15 minutes et 48 heures
                    AND o.updated_at <= NOW() - INTERVAL 15 MINUTE
                    AND o.updated_at >= NOW() - INTERVAL 48 HOUR
                    AND o.shop_id IS NOT NULL 
                    AND ccs.customer_phone IS NULL
            `;
    ...
    /**
     * Récupère la dernière commande d'un client par son numéro de téléphone.
     * Utilisé par l'IA pour trouver le marchand à contacter.
     */
    getLastOrderForCustomer: async (db, customerPhone) => {
        const connection = await db.getConnection();
        try {
            const query = `
                SELECT 
                ...
                LIMIT 1
            `;
            const [rows] = await connection.execute(query, [customerPhone]);
            return rows[0]; // Renvoie la commande (ou undefined si pas trouvée)
        } catch (error) {
            console.error("[OrderModel] Erreur getLastOrderForCustomer:", error);
            return null;
        } finally {
            connection.release();
        }
    ...
    /**
     * Vérifie si un numéro de téléphone est un client (a passé au moins une commande).
     * Utilisé par le service AI pour l'identification des rôles.
     */
    isCustomer: async (db, phoneNumber) => {
        const connection = await db.getConnection();
        try {
            const query = `
                SELECT 1
