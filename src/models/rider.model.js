...
module.exports = {
    ...
    /**
     * Trouve un livreur par son numéro de téléphone (utilisé par ai.service.js).
     * Effectue la recherche sur la table USERS car les rôles y sont définis.
     */
    getRiderByPhone: async (phoneNumber) => {
        // La fonction utilise dbConnection défini dans init()
        const query = `
            SELECT u.id, u.name, u.phone_number, u.role
            FROM users u
        ...
        try {
            ...
            console.error("Erreur SQL dans RiderModel.getRiderByPhone:", error.message);
            return null;
        }
    }
};