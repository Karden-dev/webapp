...
/**
 * Calcule la situation de la caisse complète avec les détails du CA.
...
const getCashMetrics = async (startDate, endDate) => {
    try {
        ...
        const [
            revenueData, // <-- Changé : contient maintenant { total, delivery, packaging, storage }
            cashCollected,
            cashTransactions,
            debtMetrics,
            shortfallMetrics
            ...
            cashStatModel.getCashCollected(startDate, endDate),
        ...
        // Utilise revenueData.total pour le calcul global
        const montantEnCaisse = revenueData.total 
            - sortiesNettes 
            - elementsNonRembourses 
            + elementsRembourses;

        // 3. Retourner le résultat avec les nouveaux détails du CA
        return {
            montant_en_caisse: parseFloat(montantEnCaisse.toFixed(2)), 
            
            // CA Global
            chiffre_affaire_ca: parseFloat(revenueData.total.toFixed(2)),
            
            // --- NOUVEAUX CHAMPS POUR LE DÉTAIL ---
            ca_frais_livraison: parseFloat(revenueData.delivery.toFixed(2)),
            ca_frais_emballage: parseFloat(revenueData.packaging.toFixed(2)),
            ca_frais_stockage: parseFloat(revenueData.storage.toFixed(2)),
            // --------------------------------------
